...
class GTNN_outer(torch.nn.Module):
  ...
  def __init__(self, dim, device, gs_dim, additional_feature_dim, nb_classes, use_additional_features, model_type):
      ...
      if model_type == "sage":
            ...
            self.convs.append(GCNConv(dim, gs_dim).to(device))
      elif model_type == "gatconv":
            self.convs.append(GATConv(dim, gs_dim).to(device))
